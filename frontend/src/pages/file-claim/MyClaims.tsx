import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { listCases } from "../../api";
import type { ClaimOverview } from "../../api";
import { STATUS_LABEL } from "../../lib/labels";
import { rememberedCaseIds } from "./openedClaims";
import { Failure } from "../../components/feedback/Failure";
import { TaskGate } from "../../components/workshop/TaskGate";

export function MyClaims() {
  const [claims, setCases] = useState<ClaimOverview[] | null>(null);
  const [error, setError] = useState<Error | null>(null);

  useEffect(() => {
    const mine = new Set(rememberedCaseIds());
    listCases()
      .then((all) => setCases(all.filter((c) => mine.has(c.id))))
      .catch((e: Error) => setError(e));
  }, []);

  return (
    <>
      <header>
        <h1>My claims</h1>
        <p>
          The claims you have opened. Pick one to see what it still needs and
          send more in.
        </p>
      </header>

      {error && <Failure error={error} />}

      <TaskGate
        task="DOCUMENT_AGENT"
        instead="Opening a claim works, but sending documents into one does not yet — so every claim here will sit at awaiting documents."
      >
        <TaskGate
          task="FIRST_AGENT"
          instead="This list is the claims task 1's agent opened. Until it is written there is nothing to list, however many times you try to report one."
        >
          <section className="claims">
            {claims !== null && claims.length === 0 && (
              <p className="empty">
                You have not opened any claims yet. Report a new claim to get
                started.
              </p>
            )}
            {claims?.map((c) => (
              <Link key={c.id} className="claim-row" to={`/claims/${c.id}`}>
                <span className="reference">
                  {c.typeLabel}
                  <span className="claim-reference"> · {c.reference}</span>
                </span>
                <span className={`status ${c.status.toLowerCase()}`}>
                  {STATUS_LABEL[c.status]}
                </span>
                <span className="outstanding-count">
                  {c.outstanding.length === 0
                    ? "Everything required has arrived"
                    : `Still needs ${c.outstanding.join(", ")}`}
                </span>
              </Link>
            ))}
          </section>
        </TaskGate>
      </TaskGate>
    </>
  );
}
