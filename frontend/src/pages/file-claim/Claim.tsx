...
export function Claim() {
  ...
  const intro = (useLocation().state as { intro?: CreatedClaim } | null)?.intro;
  const [overview, setOverview] = useState<ClaimOverview | null>(null);
  ...
  useEffect(() => {
    ...
    refresh();
    listDocuments()
      .then((all) => setDocuments(all.filter((d) => d.claimId === claimId)))
    ...
    <>
      {intro ? (
        <section className={`detected ${intro.confidence.toLowerCase()}`}>
          <span className="detected-label">We have opened a claim for you</span>
        ...
        <header>
          <h1>{checklist.typeLabel || "Your claim"}</h1>
          <p className="claim-reference-line">{checklist.reference}</p>
        </header>
      )}
      ...
      {checklist.requiredDocuments.length > 0 ? (
        ...
        <p className="empty">
          This claim has no set list of documents. Send in anything relevant.
        </p>
      )}
