...
An extra, and optional. Task 1 read one sentence and committed to a claim type on the spot. This one
is allowed to ask first.

**Time:** 30 minutes. **You need:** task 1 read for comparison, and task 3 working if you want to

The parts, in order, are in `backend/src/main/java/com/example/aiworkshop/tasks/task_7_create_claim_chat/README.md` — each one names the file, what it is for, and what to reach for.
...
**It chooses a scenario, not a type.** `ClaimScenario` is finer than `ClaimType`: `TRAVEL_CANCELLATION`
and `TRAVEL_BAGGAGE` are both `TRAVEL`, but each owns its own checklist. The claim is still opened as a
`ClaimType` a handler recognises — the scenario is only how the interview decides which documents to
ask for. Read the enum and note that `catalog()` is what the prompt is shown, rendered from the same
values a claim is opened from, so the taxonomy the agent reasons over cannot drift from the one it
picks from.
...
summariser in task 5 is handed a projection rather than fetching one.

## What to write

Open
`backend/src/main/java/com/example/aiworkshop/tasks/task_7_create_claim_chat/agent/ClaimIntakeInterviewer.java`
...
The rest of the feature is plumbing, in `backend/src/main/java/com/example/aiworkshop/claims/interview/`
— the controller, the `InterviewTurn` schema, the `ClaimScenario` taxonomy, and `InterviewClaimOpener`,
which writes into the same `ClaimStore` as task 1 but mints its own references so the two intakes never
...
and it should land you on a claim whose checklist matches what you said, not the whole of travel.
...
- Say something that is no kind of insurance at all. `OTHER`, not a forced travel claim.
