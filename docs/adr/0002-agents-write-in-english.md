...
exactly as they appear on it: they are evidence a Claim Handler matches against the artefact, and a
...
`ClaimSummarizer` were both told to write "in the language the document is written in";
`ClaimStatusWriter` was told nothing and wrote English by default.

The two were not equally able to obey. `DocumentAnalyzer` reads the file. `ClaimSummarizer` never
...
"Scandinavian" and wrote the Claim Summary in Swedish, beside an English analysis of the same file.
...
the only agent that can actually see the file — and it is passed to `ClaimSummarizer` as a template
variable. Rejected: it keeps the intent at the cost of a schema field, a plumbing change, and no
defined answer for a Claim holding Documents in two languages.
...
- **`ClaimSummarizer` is handed `DocumentForSummary`, not `UploadedDocument`.** What an agent is given
...
- **A cached Claim Summary outlives a prompt change.** `ClaimSummaryStore` is keyed on the set of
