...
A per-Case conversation with a tool-using agent that proposes writes.
...
A Case Handler opening a Case gets two pieces of agent prose — the situation report and the Case
...
There is also nothing a Case Handler can do from inside that reading. Confirming a Review means
...
A **Case Chat**: a conversation pinned to the one Case a Case Handler has open, in a sticky column
beside the Case's contents, so the answer and the evidence are on screen together.

The agent behind it starts with the Case at a glance — Case Status, Required Documents, what is
outstanding, the Case Summary, and a one-line index of every attached Document. Anything deeper it
...
Decline, and only a Case Handler's click makes it real. A confirmed Document Request appears on the
Claimant's upload screen.

Every tool the agent used is shown under its answer, so a Case Handler can see what the answer was
built from.

## User Stories

1. As a Case Handler, I want to ask questions about the Case I have open, so that I do not have to read every Document to answer something specific.
2. As a Case Handler, I want the chat to already know which Case I am looking at, so that I never have to name or identify the Case in my question.
3. As a Case Handler, I want the chat to sit beside the Case contents rather than covering them, so that I can read an answer and check it against the Document in the same glance.
4. As a Case Handler, I want the agent to already know the Case Status, the Required Documents and what is outstanding, so that "what is this waiting on?" is answered instantly.
5. As a Case Handler, I want the agent to already know the Case Summary, so that its answers are consistent with the prose I have just read above it.
6. As a Case Handler, I want the agent to know what Documents are attached and what each one counts as, so that I can refer to "the repair invoice" and be understood.
7. As a Case Handler, I want to ask what a specific Document says, so that I get its Extraction without opening it.
8. As a Case Handler, I want to ask why a Document was judged poor quality, so that I can decide whether to Review it without hunting for the reason on screen.
9. As a Case Handler, I want to ask the agent to go back and read the original file, so that I can get at something no Extraction captured.
10. As a Case Handler, I want the agent to tell me when the original file genuinely does not show what I asked about, so that I stop looking and chase the Claimant instead.
11. As a Case Handler, I want to ask whether the Documents disagree with each other, so that I catch a contradiction before deciding the Case.
12. As a Case Handler, I want to ask a follow-up that refers back to the previous answer, so that I can dig into something without restating it.
13. As a Case Handler, I want the conversation to still be there when I come back to the Case, so that I do not repeat work I already did.
14. As a Case Handler, I want to see which tools the agent used to answer, so that I can tell a looked-up fact from a guess.
15. As a Case Handler, I want to see which Document a looked-up fact came from, so that I can go and check it myself.
16. As a Case Handler, I want an empty chat with a few suggested questions when I open a Case, so that I know what is worth asking without waiting for a greeting.
17. As a Case Handler, I want opening a Case to cost no more than it does today, so that browsing Cases does not get slower because a chat exists.
18. As a Case Handler, I want the agent to suggest a Review when it thinks a Document is workable despite a poor Quality Assessment, so that I am not the one who has to notice.
19. As a Case Handler, I want a suggested Review to change nothing until I click Confirm, so that the Case Status never moves because a model decided it should.
20. As a Case Handler, I want to Confirm a suggested Review from inside the conversation, so that I do not have to find the Document's own button.
21. As a Case Handler, I want confirming a Review to move the Case exactly as the Document's own Review button does, so that there is one behaviour and not two.
22. As a Case Handler, I want to Decline a Proposal I disagree with, so that I can say no without ignoring it.
23. As a Case Handler, I want the agent to know I declined a Proposal, so that it does not propose the same thing again two questions later.
24. As a Case Handler, I want the agent to know which Proposals are still outstanding, so that it does not repeat one I have not got to yet.
25. As a Case Handler, I want Proposals to still be there when I reopen the Case, so that an unanswered suggestion is not silently lost.
26. As a Case Handler, I want the agent to propose asking the Claimant for a named document, so that the chase the situation report keeps telling me to do is something I can actually do.
27. As a Case Handler, I want a Document Request to reach the Claimant only after I confirm it, so that a model cannot contact a Claimant on my behalf.
28. As a Case Handler, I want confirming a Document Request not to change the Case's Required Documents, so that asking a question cannot move a Case backwards out of READY_FOR_DECISION.
29. As a Claimant, I want to see what my Case Handler has asked me for, so that I know what to upload next.
...
32. As a Case Handler, I want the agent to be unable to perform any write on its own, so that I can trust that nothing changed while I was reading.
33. As a Case Handler, I want the agent to answer in English regardless of what language the Documents are in, so that its answers match the rest of the handler side.
34. As a Case Handler, I want quoted field names and values to stay in the Document's own language, so that I can match them against the artefact.
35. As a Case Handler, I want a failed model call to tell me what actually went wrong, so that I am not staring at a blank panel.
36. As a Case Handler, I want the chat to work the same whichever model provider is configured, so that the demo is not tied to one vendor.
...
- The Case Chat is pinned to one Case. There is no cross-Case reach and no desk-level assistant.
- Calls are blocking, consistent with every other model call in the application. Streaming is not
  part of this work.
- There is no authentication, so a Case Chat belongs to the Case, not to a person. Two Case Handlers
  looking at the same Case share one conversation. This is the intended model, not a limitation.

### The seam

- `CaseDesk` remains the single seam. It grows `chat`, `proposeReview`, `proposeDocumentRequest`,
...
- The rules about what a Proposal is and what confirming one means live on the Proposal types, the
  way the rules about Case Status live on `Case`. `CaseDesk` continues to fetch, delegate and hand
  back.
- The tools class holds no logic. Every tool method delegates straight to `CaseDesk`. This is a hard
...
- **The Case Chat agent.** Takes the Case Handler's question plus the Case snapshot as template
  variables, keyed by a memory identifier. Returns `Result<String>` rather than `String`, because
  `Result` is what carries `toolExecutions()`, and the tool calls are shown to the Case Handler.
- **The document reader.** One-shot, given a file's contents and a question, returning free text. It
  is given no Case context at all — a reader that knows nothing about the Case cannot be led by it.
...
The dividing line: **the agent sees the Case at a glance and looks closer on demand.**

In the system message:

- Case reference, Case Status, Required Documents, and which of them are outstanding.
- The Case Summary, read from its existing cache. Chat reuses the cached summary and writes it only
  if absent, so a Case Handler who has already opened the Case pays nothing extra.
...
Four. Each receives the Case identifier through the memory-identifier binding rather than as a model
supplied argument, so the model cannot address another Case.
...
- Tools address Documents by **filename**, not by identifier. Identifiers are unspeakable and the
  index the agent holds is by filename. Filenames resolve within the Case; where two Documents share
  one, the most recently uploaded wins — the rule `Case` already uses to pick the counting Document.
...
- Keyed by Case identifier, which doubles as the tools' Case binding.
...
- State is `PROPOSED`, `CONFIRMED` or `DECLINED`. Proposals persist on the Case rather than expiring
  with the conversation, and both outstanding and declined ones are fed back into the system message.
- Held in an in-memory store keyed by Case, the same shape as the existing stores.
...
- A Document Request does **not** append to a Case's Required Documents. Per ADR 0001 that list is
  what Case Status is derived from, and letting an agent extend it would let a model move a Case
  backwards out of `READY_FOR_DECISION`.
- Neither confirming nor declining a Proposal invalidates the cached Case Summary, for the same
...
- One endpoint to send a chat turn for a Case, returning the answer, the tool calls made, and any
  Proposals raised.
- Endpoints to confirm and to decline a Proposal.
- Document Requests are added to the existing Case overview rather than given an endpoint of their
  own — the Claimant screen already fetches the Case list, so this costs no new call.
- Failures surface the real cause, as the existing controllers do.

### Frontend

- A sticky chat column on the Case Handler's Case screen. The Case list is untouched.
- Empty on open, with three suggested questions as clickable chips. No greeting, so opening a Case
...
The intake agent, the Case Summary agent and the situation-report agent all keep running exactly as
they do now. The Case screen must be useful before anyone types a word, and the cached Case Summary
is what grounds the chat.

## Testing Decisions

A good test here asserts external behaviour: what an agent was handed, and whether a Case actually
...
**At the `CaseDesk` seam**, with both new agents mocked exactly as the Case Summary and
situation-report agents already are, and argument captors used to inspect what they were handed:

- Proposing a Review and confirming it moves the Case; declining it leaves the Case where it was.
- A Proposal on its own, unconfirmed, changes nothing.
- A confirmed document request Proposal produces a Document Request visible on the Case overview,
  and does not alter the Case's Required Documents or its Case Status.
- Declined and outstanding Proposals reach the chat agent.
- The chat reuses a cached Case Summary rather than rewriting it.
- The reader agent is called with a file only when the read tool is exercised.

Prior art: the existing Case desk test, which mocks both handler-side agents, captures what each was
handed, and reads Case Status back through the Case list.
...
Prior art: the existing Case Summary projection test, whose javadoc gives the rule this follows — a
...
- **Three glossary terms** to add: Case Chat, Proposal, Document Request. No term for "agent" — the
