 ...
 * quietly lost.
 */
class ClaimIntakeInterviewerTest {

    private static Method next() {
        return Arrays.stream(ClaimIntakeInterviewer.class.getMethods())
        ...
        assertThat(String.join("\n", next().getAnnotation(SystemMessage.class).value()))
                .describedAs(
                        "the prompt should render {{scenarios}}, so the taxonomy the agent chooses from is the"
                                + " enum a claim is opened from rather than a second copy that goes stale")
                .contains("{{scenarios}}");
    }
