...
class ClaimFileTest {
    ...
    void aClaimWithOneUnreadableDocument() {
        // Both agents answer with a Result now, so an unstubbed mock hands back null and the cost
        // logging trips over it. A default keeps every test that does not care about the summary from
        // having to say so.
        when(summarizer.summarise(anyString(), anyList()))
                .thenReturn(Result.<String>builder().content("Two documents, and they agree.").build());
        when(statusWriter.write(anyString(), any(), anyList(), anyList()))
                .thenReturn(Result.<String>builder().content("Waiting on the claimant.").build());
        ...
        when(summarizer.summarise(anyString(), anyList())).thenReturn(Result.<String>builder().content("What the documents say, taken together.").build());
    }
