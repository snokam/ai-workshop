...
public interface ClaimSummarizer {
    ...
    @SystemMessage(
            ...
            The expensive agent: every document on a claim, read together, in one prompt on every screen load.
              ...
              summarise(@V("claimType") String claimType, @V("documents") List<DocumentForSummary> documents)
    ...
    @UserMessage(
            ...
            {{documents}}
            """)
    String summarise(@V("claimType") String claimType, @V("documents") List<DocumentForSummary> documents);
}
